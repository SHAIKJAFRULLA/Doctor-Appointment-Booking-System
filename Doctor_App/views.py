...
def New_Booking(request):
	if request.method == "POST":
		...
		# Check if the doctor, slot and date are already booked
		if Booking.objects.filter(Doctor_Id=Doctor_ID, Slot=slot, Date=date).exists():
			messages.info(request, "Slot already booked")
		elif Booking.objects.filter(Doctor_Id=Doctor_ID, Patient_Id=Patient_Id, Slot=slot, Date=date).exists():
			messages.info(request, "You have already booked an appointment for this date and time")
		else:
